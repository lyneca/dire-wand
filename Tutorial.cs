...
public TutorialSave() {
    ...
    foreach (var spell in spells) {
        spellsCast[spell.tier][spell.id] = false;
    }
}
...
public class Tutorial : WandSkill {
...
public override CatalogData Clone() {
    if (base.Clone() is not Tutorial clone) return null;
    if (tutorials != null)
        clone.tutorials = [..tutorials];
    return clone;
}
...
public IEnumerator LoadSave() {
    ...
    for (var i = 0; i < TutorialSave.local.numTiers; i++) {
        foreach (var spell in wand.spells) {
            ...
            string spellTitle = spell.id ?? spell.GetType().Name;
            tutorials.Add(new TutorialData {
                id = "Tutorial: " + spellTitle,
                title = spellTitle,
                description = spell.description,
                color = spell.color,
                iconAddress = spell.orbIconAddress,
                tier = spell.tier,
